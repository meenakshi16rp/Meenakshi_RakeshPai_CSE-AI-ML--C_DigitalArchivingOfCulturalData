from flask import Flask, render_template, redirect, url_for, request, flash, jsonify, send_file, session, g
...
def analyze_image():
    ...
    Analyze the image and provide a comprehensive response with the following structure:
    
    1. Title: A concise name of what's shown in the image (monument name, dance form, art style, person's name)
    2. Description: A detailed paragraph (100-150 words) that explains the cultural significance, historical context, 
       and key features visible in the image. For monuments, include location and era. For dance forms, include region of origin 
       and distinguishing characteristics. For historical figures, include their significance and contributions.
    ...
    response = client.chat.completions.create(
        ...
        max_tokens=300,  # Increased to allow for longer descriptions
        temperature=0.7,
        top_p=0.7,
        top_k=50,
    ...
    try:
        ...
        description = desc_match.group(1).strip() if desc_match else response_text
